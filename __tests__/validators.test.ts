...
describe("WatchlistItemSchema", () => {
  ...
  it("rejects missing required fields", () => {
    ...
    expect(
      WatchlistItemSchema.safeParse({ ...validItem, mediaType: undefined })
        .success,
    ).toBe(false);
    expect(
      WatchlistItemSchema.safeParse({ ...validItem, status: undefined })
        .success,
    ).toBe(false);
  });
  ...
  it("rejects invalid mediaType", () => {
    expect(
      WatchlistItemSchema.safeParse({ ...validItem, mediaType: "book" })
        .success,
    ).toBe(false);
  });
  ...
  it("rejects invalid status", () => {
    expect(
      WatchlistItemSchema.safeParse({ ...validItem, status: "done" }).success,
    ).toBe(false);
  });
});
