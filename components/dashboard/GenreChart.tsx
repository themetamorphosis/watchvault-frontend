...
import { Compass, Film, Tv, Sparkles, TrendingUp, BarChart2, Eye, Clock, Heart } from "lucide-react";
...
export default function GenreChart({ stats, topGenres, decadesData, activeTab, setActiveTab }: GenreChartProps) {
    return (
                ...
                <div className="min-h-[200px]">
                    {activeTab === "genres" && (
                        <div className="space-y-3">
                            {topGenres.length > 0 ? (
                                topGenres.map(g => {
                                    const maxCount = Math.max(...topGenres.map(x => x.count), 1);
                    ...
                    {activeTab === "status" && (
                        <div className="grid grid-cols-3 gap-3">
                            <div className="text-center p-4 rounded-2xl bg-white/5 border border-white/5">
                                ...
                                <div className="text-lg font-bold text-white"><CountUp end={stats.watchedCount} duration={1} /></div>
                                <div className="text-[9px] text-neutral-400 font-mono uppercase mt-1">Watched</div>
                            </div>
                            <div className="text-center p-4 rounded-2xl bg-white/5 border border-white/5">
                                <Clock className="h-5 w-5 text-amber-400 mx-auto mb-2" />
                                <div className="text-lg font-bold text-white"><CountUp end={stats.pendingCount} duration={1} /></div>
                                <div className="text-[9px] text-neutral-400 font-mono uppercase mt-1">Pending</div>
                            </div>
                            <div className="text-center p-4 rounded-2xl bg-white/5 border border-white/5">
                                <Heart className="h-5 w-5 text-violet-400 mx-auto mb-2" />
                                <div className="text-lg font-bold text-white"><CountUp end={stats.wishlistCount} duration={1} /></div>
                                <div className="text-[9px] text-neutral-400 font-mono uppercase mt-1">Wishlist</div>
                            </div>
                        </div>
                    )}
