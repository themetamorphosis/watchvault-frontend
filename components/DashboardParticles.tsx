...
function Particles({ count = 250 }: { count?: number }) {
    ...
    // Handle mouse movement to sway the particle field
    useEffect(() => {
    ...
    const [positions, sizes, colors] = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Varied colors from our theme palette
    ...
    useFrame((state, delta) => {
        ...
        pointsRef.current.rotation.y = state.clock.elapsedTime * 0.015 + mouse.current.x;
        pointsRef.current.rotation.x = state.clock.elapsedTime * 0.008 + mouse.current.y;
    });
