      ...
      >
        <div className="mx-auto max-w-[1440px] px-6 lg:px-10 h-20 flex items-center justify-between gap-4">
          ...
          {isRetro ? (
            ...
            >
              <span className="text-[#f0a500] dark:text-tui-amber font-bold text-sm md:text-base">
                guest@watchvault
              </span>
              <span className="text-nav-text opacity-60 text-sm md:text-base">
                :
              </span>
              <span className="text-blue-400 font-bold text-sm md:text-base">
                ~
              </span>
              <span className="text-nav-text font-bold text-sm md:text-base">
                $
              </span>
            </Link>
          ) : (
              ...
              {NAV_TABS.map((tab, idx) => {
                ...
                return (
                  <React.Fragment key={tab.href}>
                    {isRetro && idx > 0 && (
                      <span className="text-nav-border px-1.5 select-none">
                        |
                      </span>
                    )}
                            ...
                            {tab.subTabs.map((sub) => {
                              ...
                              return (
                                <Link
                                  ...
                                  className={
                                    isRetro
                                      ? `flex items-center gap-2 px-3 py-2 transition-colors uppercase ${
                                          isSubActive
                                            ? "bg-nav-hover-bg text-nav-active-bg border-l-2 border-nav-active-bg font-bold"
                                            : "text-nav-text hover:text-nav-hover-text hover:bg-nav-hover-bg"
                                        }`
                                      : `flex items-center gap-2 px-3.5 py-2 transition-colors rounded-xl text-xs md:text-sm font-medium ${
                                          isSubActive
            ...
            <div className="relative" ref={themeRef}>
              {isRetro ? (
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-3 w-3 opacity-60 transition-transform ${themeOpen ? "rotate-180" : ""}`}
                  />
                </button>
              )}
                ...
                {themeOpen && (
                  ...
                  >
                    {THEMES.map((t) => (
                      ...
                      >
                        <t.icon
                          className={`h-4 w-4 ${isRetro ? "" : theme === t.value ? "text-tui-amber" : "text-tui-text-muted group-hover:text-tui-text"}`}
                        />
                        <span className={isRetro ? "" : "text-sm"}>
                          {isRetro ? `[ ${t.label} ]` : t.label}
                        </span>
                        {!isRetro && theme === t.value && (
                          <span className="ml-auto text-xs text-tui-text-muted font-sans">
                            Active
                          </span>
                        )}
                      </button>
                      ...
                      {isRetro ? (
                        <>
                          <div className="px-3 py-2 border-b border-nav-border mb-1">
                            <div className="text-[10px] text-nav-text opacity-80 font-mono">
                              EMAIL
                            </div>
                            <div className="text-nav-text-muted truncate text-[11px] font-mono">
                              {session.user?.email}
                            </div>
                          </div>
                          <Link
                        ...
                        <>
                          <div className="px-4 py-3 border-b border-tui-border mb-1.5">
                            <div className="text-[10px] text-tui-text-muted font-semibold uppercase tracking-wider">
                              EMAIL
                            </div>
                            <div className="text-tui-text truncate text-xs mt-0.5">
                              {session.user?.email}
                            </div>
                          </div>
                          <Link
                ...
                {NAV_TABS.map((tab) => {
                  ...
                  return (
                    <Link
                      ...
                      className={
                        isRetro
                          ? `flex items-center gap-3 px-4 py-3 border border-transparent transition-colors ${
                              isActive
                                ? "bg-nav-active-bg text-nav-active-text font-bold"
                                : "text-nav-text hover:text-nav-hover-text hover:bg-nav-hover-bg"
                            }`
                    ...
                    >
                      ...
                      {tab.comingSoon && (
                        <span
                          className={
                            isRetro
                              ? "ml-auto text-[9px] text-nav-text-muted"
                              : "ml-auto text-[9px] text-tui-text-muted"
                          }
                        >
                          {isRetro ? "[ DEV ]" : "DEV"}
                        </span>
