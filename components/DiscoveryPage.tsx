...
import { MOVIE_GENRES, TV_GENRES, DECADES, SORT_OPTIONS, type ListType } from "@/lib/discovery-constants";
        ...
        <div className={`w-full min-h-screen bg-tui-bg text-tui-text ${
            ...
            <div className="relative z-10 mx-auto max-w-[1440px] px-6 lg:px-10 pt-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center border border-tui-border bg-tui-panel/40 backdrop-blur-md p-3 gap-3 mb-6 rounded-xl">
                    ...
                    <div role="radiogroup" aria-label="Media type" className="flex gap-1 bg-tui-bg/50 border border-tui-border/50 p-1 font-mono text-xs rounded-lg">
                ...
                <div className="flex flex-col lg:flex-row gap-6 items-start text-left">
                    ...
                    {activeTab === "explorer" && showFilters && (
                        <ExplorerFilters
                            ...
                            onGenreChange={(v) => { setGenreFilter(v); setPage(1); }}
                            onDecadeChange={(v) => { setDecadeFilter(v); setPage(1); }}
                            onSortChange={(v) => { setSortBy(v); setPage(1); }}
                            onReset={() => { setGenreFilter(""); setDecadeFilter(""); setSortBy("popularity.desc"); }}
                        />
                    )}
...
<SkeletonGrid count={15} />
                                    ...
                                    {results.map((item, index) => (
                                        <div key={item.tmdbId} className="relative">
                                            ...
                                            <MediaDiscoverCard 
                                                ...
                                                onAdd={(status: Status) => handleAddToWatchlist(item, status)}
            ...
            <DetailOverlay
                ...
                onClose={() => { setSelectedItem(null); setDetails(null); }}
                onRate={handleRate}
