...
export default function MediaDiscoverCard({
  item,
  isRetro,
  onOpen,
  onAdd,
  ...
  const meta = [item.year ? String(item.year) : "", item.genres?.[0] || ""]
    .filter(Boolean)
    .join(" • ");
  ...
  if (isRetro) {
    ...
    return (
      <div
        ...
        className={`border bg-tui-panel ${borderClass} font-mono text-left flex flex-col h-full`}
      >
        <div
          onClick={onOpen}
              ...
              <div className="relative" ref={menuRef}>
                ...
                {quickMenuOpen && (
                  <div className="absolute left-0 bottom-full mb-1.5 z-40 bg-tui-panel border border-tui-border p-1 w-24 flex flex-col gap-0.5 shadow-2xl">
                    {(["watched", "pending", "wishlist"] as Status[]).map(
                      (status) => (
                        <button
                          key={status}
                          onClick={() => {
  ...
  return (
    <div
      className="group relative cursor-pointer select-none text-left rounded-xl overflow-hidden border border-white/5 hover:border-white/10 shadow-lg transition-all duration-300"
      onClick={onOpen}
    >
      {/* Poster Image */}
