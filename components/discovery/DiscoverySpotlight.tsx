                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="max-w-4xl text-left space-y-5">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {spotlightItem.genres.slice(0, 3).map((g) => (
                                    <span key={g} className="px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-[10px] text-zinc-300 font-mono uppercase">
                                        {g}
                                    </span>
                                ))}
                            </div>
                        ...
                        {spotlightItem.posterUrl && (
                            ...
                            >
                                <TMDBImage src={spotlightItem.posterUrl} alt={spotlightItem.title} width={180} height={270} className="w-full rounded-xl border border-white/10 shadow-2xl" />
                            </div>
                        )}
                    </motion.div>
                </AnimatePresence>
                ...
                <div className="border border-tui-border bg-tui-panel p-4 font-mono text-xs space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="md:col-span-1 border border-tui-border bg-tui-bg aspect-[2/3] overflow-hidden">
                            {spotlightItem.posterUrl ? (
                                <TMDBImage src={spotlightItem.posterUrl} alt={spotlightItem.title} fill className="w-full h-full object-cover filter grayscale" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-[10px] text-tui-text-muted uppercase">No Image</div>
                            )}
                        </div>
                        <div className="md:col-span-3 space-y-2">
                            <div className="text-tui-text font-bold uppercase text-sm">{spotlightItem.title}</div>
                            <div className="text-tui-text-muted">{spotlightItem.year || "N/A"} {spotlightItem.voteAverage ? `| RATING: ${spotlightItem.voteAverage.toFixed(1)}` : ""}</div>
                            <div className="text-tui-text-muted leading-relaxed line-clamp-3">{spotlightItem.overview || "No description available."}</div>
                            <div className="flex gap-2 pt-2">
                                {watchlistItem ? (
                                    <button onClick={() => onRemove(watchlistItem.id)} className="px-3 py-1 border border-red-950 text-red-500 bg-red-950/10 hover:border-red-600 uppercase text-[10px]">
                                        [DEL]
                                    </button>
                                ) : (
                                    <>
                                        <button onClick={() => onAdd(spotlightItem, "watched")} className="px-3 py-1 border border-tui-border text-tui-green hover:border-tui-green uppercase text-[10px]">
                                            [+WATCHED]
                                        </button>
                                        <button onClick={() => onAdd(spotlightItem, "wishlist")} className="px-3 py-1 border border-tui-border text-tui-text-muted hover:border-tui-amber uppercase text-[10px]">
                                            [+WISHLIST]
                                        </button>
                                    </>
                                )}
