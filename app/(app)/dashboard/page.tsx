...
const DashboardParticles = dynamic(
  () => import("@/components/DashboardParticles"),
  { ssr: false },
);
...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"genres" | "status" | "decades">(
    "genres",
  );
  const [selectedAddResult, setSelectedAddResult] =
  ...
  const activeSpotlight = useMemo(() => {
    if (shuffledId) {
      const f = items.find((i) => i.id === shuffledId);
      if (f) return f;
    }
  ...
  useEffect(() => {
    ...
    stats.favoritesList
      .filter(
        (it) =>
          !syncingRefs.current.has(it.id) &&
          !(it.coverUrl && it.genres?.length && it.description),
      )
      .slice(0, 3)
      .forEach((it) => {
  ...
  const handleShuffle = () => {
    ...
    const pool = activeSpotlight
      ? items.filter((i) => i.id !== activeSpotlight.id)
      : items;
    setShuffledId(pool[Math.floor(Math.random() * pool.length)].id);
  };
  ...
  const handleConfirmQuickAdd = async () => {
    ...
    const starsStr =
      quickAddRating > 0
        ? "★".repeat(quickAddRating) + "☆".repeat(5 - quickAddRating) + " "
        : "";
    await handleUpsert({
      title: selectedAddResult.title,
      mediaType: selectedAddResult.mediaType as MediaType,
      status: quickAddStatus,
      favorite: quickAddFav,
      year: selectedAddResult.year ?? undefined,
      coverUrl:
        selectedAddResult.posterUrl?.replace("/w185/", "/w780/") ?? undefined,
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10 py-8 font-sans space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-28 bg-tui-panel border border-tui-border rounded-2xl"
            />
          ))}
        </div>
      </div>
  ...
  if (isRetro) {
    return (
      <RetroDashboard
        ...
        onQuickAdd={(r) => setSelectedAddResult(r)}
        toast={toast}
      />
    );
  }
    ...
    <div className="relative min-h-screen w-full bg-[#030303] text-white font-sans overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-6">
          ...
          <MediaDistribution
            movieCount={stats.counts.movie}
            tvCount={stats.counts.tv}
            animeCount={stats.counts.anime}
          />
          ...
          <div className="col-span-full md:col-span-3 bg-neutral-900/30 border border-white/10 rounded-3xl p-6 flex flex-col justify-between transition-all duration-500 hover:border-white/15 hover:shadow-[0_24px_80px_rgba(0,0,0,0.5)]">
            <div>
              <h3 className="text-xs font-bold text-neutral-300 uppercase tracking-widest flex items-center gap-1.5 mb-1">
                <Plus className="h-4 w-4 text-amber-400" /> Quick Library Add
              </h3>
              ...
              <div className="relative z-40">
                <TmdbSearchInput
                  mediaType="movie"
                  onSelect={(r) => setSelectedAddResult(r)}
        ...
        <QuickAddModal
          selectedAddResult={selectedAddResult}
          quickAddStatus={quickAddStatus}
          quickAddFav={quickAddFav}
          quickAddRating={quickAddRating}
          quickAddNotes={quickAddNotes}
          setStatus={setQuickAddStatus}
          setFav={setQuickAddFav}
          setRating={setQuickAddRating}
          setNotes={setQuickAddNotes}
          onConfirm={handleConfirmQuickAdd}
          onCancel={() => setSelectedAddResult(null)}
        />
        <AnimatePresence>
          {toast && (
            <motion.div
